...
def conv3x3_qmaybe(
        ...
        groups: int = 1,
        dilation: int = 1,
        nonq_class: Type[nn.Module]=None,
    ...
    if quantize:
        # NOTE for now, only quantize some of the big 3x3, stride 1 layers
        assert stride == 1, stride
        mod = functool.partial(qconv_class, qkwargs=qkwargs)
    else:
        mod = nn.Conv2d
    conv = mod(
        in_planes,
...
class BasicBlock(nn.Module):
    ...
    def __init__(
        ...
        self.conv1 = conv3x3_qmaybe(inplanes, planes, stride,
            nonq_class=nonq_class, qconv_class=qconv_class, qkwargs=qkwargs)
...
class Bottleneck(nn.Module):
    ...
    def __init__(
        ...
        self.conv1 = conv3x3_qmaybe(widdth, width, stride, groups, dilation,
            nonq_class=nonq_class, qconv_class=qconv_class, qkwargs=qkwargs,
            quantize=stride==1)
...
class ResNet(nn.Module):
    def __init__(
        ...
        self._nonq_class = nonq_class
        self._qconv_class = qconv_class
        self._qwargs = qkwargs
    ...
    def _make_layer(
        ...
        layers.append(
            block(
                self.inplanes, planes, stride, downsample, self.groups, self.base_width, previous_dilation, norm_layer, nonq_class=nonq_class, qconv_class=qconv_class, qkwargs=self._qkwargs
            )
        )
