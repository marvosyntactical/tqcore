...
class QMultiHeadedAttention(nn.Module):
    ...
    def __init__(self, num_heads: int, dim: int, dropout: float = 0.1, **qkwargs):
        ...
        self.post_kq = QListener(self.k_layer, **qkwargs)

        self.v_layer = nn.Linear(dim, num_heads * head_size)
        self.post_vq = QListener(self.v_layer, **qkwargs)

        self.q_layer = nn.Linear(dim, num_heads * head_size)
        self.post_qq = QListener(self.q_layer, **qkwargs)

        self.scale = QMul(**qkwargs)
        self.scaleq = QListener(self.scale, **qkwargs)

        self.qkMatMul = QMatMul(**qkwargs)
...
class QPositionwiseFeedForward(nn.Module):
    ...
    def __init__(self, input_size, ff_size, time_window, dropout=0.1, activ=nn.ReLU6, **qkwargs):
        """
...
class QTransformerEncoderLayer(nn.Module):
    ...
    def __init__(self,
        ...
        self.qRes1 = QListener(
            self.add1, self.src_src_att.output_layer,
            **qkwargs)

        self.batch_norm1 = QBatchNorm1dTranspose(dim, momentum=bn_mom, **qkwargs)
        self.bn1q = QListener(self.batch_norm1, **qkwargs)
        ...
        self.qRes2 = QListener(self.add2, **qkwargs)
        self.batch_norm2 = QBatchNorm1dTranspose(dim, momentum=bn_mom, **qkwargs)
        self.bn2q = QListener(self.batch_norm2, **qkwargs)
